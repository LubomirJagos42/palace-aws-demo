...
class BasicMfemMesher:
    ...
    def addStepfile(self, name, stepfile, priority=-1):
        ...
        self.geometryObjectList.append({"name": name, "dimtags": self.importStepFileAndGetAllNewEntities(stepfile), "priority": priority})

    def addGmshObjectUsingDimtags(self, name, dimtags, priority=-1):
        if priority == -1:
            priority = self.internalGeometryObjectIndexCounter
            self.internalGeometryObjectIndexCounter += 1
    ...
    def addGmshVolumeObject(self, name, gmshObjectTag, priority=-1):
        ...
        self.addGmshObjectUsingDimtags(name, dimtags, priority)
    ...
    def getGeometryObject(self, name):
    ...
    def performFragmentationAndReassignTags(self):
