...
geometryOrderedList.append({"name": "coil", "dimtags": importStepFileAndGetAllNewEntities('stepfiles/coil.step')})
...
#
airboxObj = gmsh.model.occ.addSphere(0,0,0, 40)
...
#
print("Cutting coil from airbox...")
out_air, map_air = gmsh.model.occ.cut(
    ...
    [ (gmshTuple[0], gmshTuple[1]) for gmshTuple in getGeometryObject(geometryOrderedList, "coil")["dimtags"] if gmshTuple[0] == 3],
    removeObject=True,
    removeTool=False  # Keep substrate
)

print("Cutting coil from port face...")
out_port, map_port = gmsh.model.occ.cut(
    ...
    [ (gmshTuple[0], gmshTuple[1]) for gmshTuple in getGeometryObject(geometryOrderedList, "coil")["dimtags"] if gmshTuple[0] in [2,3]],
    removeObject=True,
    removeTool=False  # Keep substrate
)
...
all_surfaces = []
all_surfaces.extend([tag for dim, tag in getGeometryObject(geometryOrderedList, "coil")["dimtags"] if dim == 2])
...
createGroup(geometryOrderedList, "coil", "coil", 2, groupTag=300)
