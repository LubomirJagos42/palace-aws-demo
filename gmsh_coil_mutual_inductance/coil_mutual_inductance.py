...
# geometryOrderedList.append({"name": "coil_top", "dimtags": importStepFileAndGetAllNewEntities('stepfiles/top_coil.step')})
# # geometryOrderedList.append({"name": "coil_bottom", "dimtags": importStepFileAndGetAllNewEntities('stepfiles/bottom_coil.step')})
# geometryOrderedList.append({"name": "coil_core", "dimtags": importStepFileAndGetAllNewEntities('stepfiles/coil_core.step')})
# geometryOrderedList.append({"name": "port_in", "dimtags": importStepFileAndGetAllNewEntities('stepfiles/port_in.step')})
# geometryOrderedList.append({"name": "port_out", "dimtags": importStepFileAndGetAllNewEntities('stepfiles/port_out.step')})


# geometryOrderedList.append({"name": "airbox", "dimtags": importStepFileAndGetAllNewEntities('stepfiles2/airbox.step')})
geometryOrderedList.append({"name": "coil_top", "dimtags": importStepFileAndGetAllNewEntities('stepfiles2/top_coil.step')})
geometryOrderedList.append({"name": "coil_bottom", "dimtags": importStepFileAndGetAllNewEntities('stepfiles2/bottom_coil.step')})
geometryOrderedList.append({"name": "coil_core", "dimtags": importStepFileAndGetAllNewEntities('stepfiles2/coil_core.step')})
geometryOrderedList.append({"name": "port_in", "dimtags": importStepFileAndGetAllNewEntities('stepfiles2/port_in.step')})
geometryOrderedList.append({"name": "port_out", "dimtags": importStepFileAndGetAllNewEntities('stepfiles2/port_out.step')})
...
#
airboxObj = gmsh.model.occ.addSphere(0,0,0, 100)
...
print("Cutting coil core from airbox...")
out_port, map_port = gmsh.model.occ.cut(
    [ (gmshTuple[0], gmshTuple[1]) for gmshTuple in getGeometryObject(geometryOrderedList, "airbox")["dimtags"] if gmshTuple[0] in [3]],
    [ (gmshTuple[0], gmshTuple[1]) for gmshTuple in getGeometryObject(geometryOrderedList, "coil_core")["dimtags"] if gmshTuple[0] in [3]],
    removeObject=True,
    removeTool=False  # Keep substrate
)

print("Cutting coil from port face (bottom)...")
out_port, map_port = gmsh.model.occ.cut(
    [ (gmshTuple[0], gmshTuple[1]) for gmshTuple in getGeometryObject(geometryOrderedList, "port_in")["dimtags"] if gmshTuple[0] in [2,3]],
    [ (gmshTuple[0], gmshTuple[1]) for gmshTuple in getGeometryObject(geometryOrderedList, "coil_bottom")["dimtags"] if gmshTuple[0] in [2,3]],
    removeObject=True,
    removeTool=False  # Keep substrate
)

print("Cutting coil from port face (top)...")
out_port, map_port = gmsh.model.occ.cut(
    [ (gmshTuple[0], gmshTuple[1]) for gmshTuple in getGeometryObject(geometryOrderedList, "port_out")["dimtags"] if gmshTuple[0] in [2,3]],
    [ (gmshTuple[0], gmshTuple[1]) for gmshTuple in getGeometryObject(geometryOrderedList, "coil_top")["dimtags"] if gmshTuple[0] in [2,3]],
    removeObject=True,
    removeTool=False  # Keep substrate
)
...
all_surfaces = []
all_surfaces.extend([tag for dim, tag in getGeometryObject(geometryOrderedList, "coil_top")["dimtags"] if dim == 2])
all_surfaces.extend([tag for dim, tag in getGeometryObject(geometryOrderedList, "coil_bottom")["dimtags"] if dim == 2])
...
gmsh.model.mesh.field.setNumber(field_threshold, "SizeMin", 0.5)  # Fine near surfaces
# gmsh.model.mesh.field.setNumber(field_threshold, "SizeMax", 5.0)  # Coarse far away
gmsh.model.mesh.field.setNumber(field_threshold, "SizeMax", 15.0)  # Coarse far away
gmsh.model.mesh.field.setNumber(field_threshold, "DistMin", 0)
gmsh.model.mesh.field.setNumber(field_threshold, "DistMax", 10.0)
...
createGroup(geometryOrderedList, "airbox", "airbox", 3, groupTag=100)
createGroup(geometryOrderedList, "coil_core", "coil_core", 2, groupTag=200)
createGroup(geometryOrderedList, "coil_top", "coil_top", 2, groupTag=300)
createGroup(geometryOrderedList, "coil_bottom", "coil_bottom", 2, groupTag=400)
createGroup(geometryOrderedList, "port_in", "port_in", 2, groupTag=500)
createGroup(geometryOrderedList, "port_out", "port_out", 2, groupTag=600)
